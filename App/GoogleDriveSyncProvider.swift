...
/// Google Drive-backend för `SyncProvider`, mot `appDataFolder`-scopet
/// (en dold mapp knuten till appen — aldrig hela Drive). Google har ingen
/// path-baserad fil-API som Dropbox, så filen måste hittas via sökning
/// (`findFileID`) innan den kan läsas/uppdateras. Krypterar/dekrypterar med
/// samma `SyncCrypto` som `EncryptedFolderSyncProvider`/`DropboxSyncProvider`.
///
...
struct GoogleDriveSyncProvider: SyncProvider {
    ...
    private func token() throws -> String {
        try OAuthTokenStore.validAccessToken(for: OAuthProviders.googleDrive)
    }
}
#endif
